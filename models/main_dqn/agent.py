...
from models.main_dqn.replay_buffer import ReplayBuffer
...
class MainDQNAgent:
    def __init__(
        ...
        self.model = build_main_dqn(variant_id=variant_id)
        self.target_model = build_main_dqn(variant_id=variant_id)
    ...
    def train_step(self, global_step: int) -> Optional[float]:
        ...
        if self.variant_id == "dueling_double":
            ...
            next_q = target_next_q[np.arange(len(next_actions)), next_actions]
        else:
    ...
    def save_optimizer_state(self, path: str) -> bool:
        try:
            # Optimizer variables are serializable via numpy object array.
    ...
    def load_optimizer_state(self, path: str) -> bool:
        try:
            ...
            # Build optimizer slots if needed.
